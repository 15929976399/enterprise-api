...
(function ($) {
	...
	$.jqPagination = function (el, options) {
		...
		base.init = function () {
			...
			base.$input.on('blur.jqPagination keydown.jqPagination', function (event) {
				...
				if (event.type === 'blur') {
					...
					if(base.options.current_page!=current_page){
						tosubmit(base.options.current_page);
					}
				}
		...
		base.setInputValue = function (page) {
			...
			var	max_page	= base.options.max_page;
	
			// this looks horrible :-(
			page_string = page_string.replace('{current_page}', page);
			page_string = page_string.replace('{max_page}', max_page);
		...
		base.setLinks = function (page) {
			...
			if (link_string !== '') {
				...
				base.$el.find('a.first').attr('topage', link_string.replace('{page_number}', '1'));
				base.$el.find('a.prev, a.previous').attr('topage', link_string.replace('{page_number}', previous));
				base.$el.find('a.next').attr('topage', link_string.replace('{page_number}', next));
				base.$el.find('a.last').attr('topage', link_string.replace('{page_number}', max_page));
